  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <h4 style={{ fontSize: 12, fontWeight: 700, color: '#a0936e', textTransform: 'uppercase', letterSpacing: 1 }}>故事列表</h4>
        <Button size="small" onClick={onCreateStory}>+ 新建</Button>
      </div>

      {stories.length === 0 && <p style={{ fontSize: 12, color: '#c4b89e' }}>暂无故事</p>}
      ...
      {stories.map((s) => (
        <div
          ...
          style={{
            ...
            background: activeStory?.id === s.id ? '#B7C6E5' : 'transparent',
            color: activeStory?.id === s.id ? '#fff' : '#8a7b66',
            fontSize: 13,
          }}
        >
      ...
      {activeStory && (
        <div style={{ marginTop: 16 }}>
          <div
            ...
            style={{ fontSize: 12, fontWeight: 700, color: '#a0936e', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}
          >
            {showChapters ? '▼' : '▶'} 章节
          </div>
          {showChapters && activeStory.chapters.length === 0 && (
            <p style={{ fontSize: 12, color: '#c4b89e' }}>暂无章节</p>
          )}
          {showChapters && activeStory.chapters.map((ch) => (
            <div key={ch.number} style={{ padding: '4px 12px', fontSize: 12, color: '#9f927d', cursor: 'pointer' }}>
              第 {ch.number} 章
