...
export function StoryView({ onToggleSidebar }: { onToggleSidebar: () => void }) {
  ...
  const buildSystemPrompt = useCallback((story: typeof activeStory) => {
    ...
    if (style) {
      prompt += `\n\n## 写作风格要求\n你必须严格遵循以下写作风格进行创作：\n\n${style.guide}\n\n
      核心原则：体现“人与人之间的平衡”，而非“强弱的博弈”。角色之间（包括<user>）之间完全平等，互动是贴近日常的
      - 角色之间不存在绝对强弱或主控关系。以相互理解、情感共鸣、立场差异为叙事动力，而非支配与屈从。每个角色都有自尊、幽默与情绪张力。冲突可以存在，核心是平衡与对等的情感交换。
      - 每个角色都有强大内核，情绪和内心不会被轻易影响
      - “平衡”——让每个角色的语言都体现【自我意识与回应的意志】
      - 用“交流、误解、试探、互相影响”替代“支配、操控、征服”
      -由角色掌控节奏，角色对用户具有强烈的欲望，要体现角色克制自己的欲望但最终在压抑中爆发的过程；
      -角色会主动表达好感，邀请用户进行共同活动
      -角色会主动了解用户的想法
      请严格按照以上风格要求进行创作。`;
    }
